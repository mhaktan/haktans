        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="text" label="Reporter Name *" value={String(form.reporterName ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('reporterName', v))(e.detail)} />
                </div>
                <div>
                ...
                <div>
                  <LookupSelect label="Aircraft *" resource="Aircraft" value={String(form.aircraftId ?? '')} onChange={(v) => setField('aircraftId', v)} displayField="registration" />
                </div>
            </div>
          </div>
